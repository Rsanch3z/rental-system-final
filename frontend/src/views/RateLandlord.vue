<template>
  <div class="review-container">
    <h2 class="title">🌟 評價房東</h2>
    <form @submit.prevent="submitReview" class="form">
      <label class="label">房東 ID</label>
      <input
        v-model.number="review.landlordId"
        type="number"
        class="input"
        placeholder="請輸入房東 ID"
        required
      />
...
const submitReview = async () => {
  try {
    const payload = {
      ...
      reviewerType: "tenant",
      targetType: "landlord",
      targetId: review.value.landlordId,
    ...
    review.value = { landlordId: null, rating: 5, comment: "" }
  } catch (err) {
